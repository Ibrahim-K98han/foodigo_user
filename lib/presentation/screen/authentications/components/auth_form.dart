...
import 'package:foodigo/features/restaurant_features/Login/bloc/restaurant_login_state.dart';
import 'package:foodigo/features/restaurant_features/Login/model/restaurant_login_state_model.dart';
import 'package:foodigo/presentation/core/routes/route_names.dart';
import 'package:foodigo/presentation/restaurant_screen/authentications/components/social_btn_section.dart';
...
class _AuthFormState extends State<AuthForm> {
  ...
  late RestaurantLoginBloc restaurantLoginBloc;

  @override
  void initState() {
    loginBloc = context.read<LoginBloc>();
    restaurantLoginBloc = context.read<RestaurantLoginBloc>();
    super.initState();
  }
                    ...
                    return Column(
                      ...
                      children: [
                        ...
                        if (login is LoginStateFormValidate &&
                            login.errors.email.isNotEmpty)
                          FetchErrorText(text: login.errors.email.first),
                      ],
                    );
                  },
                )
                : BlocBuilder<RestaurantLoginBloc, RestaurantLoginStateModel>(
                  builder: (context, state) {
                    final resLogin = state.restaurantLoginState;
                    return Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        CustomFormWidget(
                          ...
                          child: TextFormField(
                            initialValue: state.email,
                            onChanged:
                                (value) => restaurantLoginBloc.add(
                                  RestaurantLoginEventUserEmail(value),
                                ),
                            decoration: InputDecoration(
                              filled: true,
                              fillColor: const Color(0xffF8FAFC),
                        ...
                        if (resLogin is RestaurantLoginStateFormValidate &&
                            resLogin.errors.email.isNotEmpty)
                          FetchErrorText(text: resLogin.errors.email.first),
                      ],
                    );
                  },
                ),
                  ...
                  builder: (context, state) {
                    return CustomFormWidget(
                      ...
                      child: TextFormField(
                        ...
                        onChanged:
                            (value) => restaurantLoginBloc.add(
                              RestaurantLoginEventPassword(value),
                            ),
                        decoration: InputDecoration(
                          filled: true,
                          fillColor: const Color(0xffF8FAFC),
                          ...
                          suffixIcon: IconButton(
                            splashRadius: 18.0,
                            onPressed: () {
                              restaurantLoginBloc.add(
                                RestaurantShowPasswordEvent(state.show),
                              );
                            },
                            icon: Icon(
                              state.show
                        ...
                        Row(
                          children: [
                            Checkbox(
                              ...
                              onChanged:
                                  (v) => loginBloc.add(LoginEventRememberMe()),
                            ),
                            const CustomText(
                              text: 'Remember me',
                              color: greyColor,
                  ...
                  builder: (context, login) {
                    ...
                    return PrimaryButton(
                      ...
                      onPressed: () {
                        ...
                        loginBloc.add(LoginEventSubmit(isUser: widget.isUser));
                      },
                    );
                  },
                )
                : BlocConsumer<RestaurantLoginBloc, RestaurantLoginStateModel>(
                  listener: (context, login) {
                    final state = login.restaurantLoginState;
                    if (state is RestaurantLoginStateError) {
                      Utils.failureSnackBar(context, state.message);
                    } else if (state is RestaurantLoginStateLoaded) {
                      Navigator.pushNamedAndRemoveUntil(
                        context,
                  ...
                  builder: (context, login) {
                    final state = login.restaurantLoginState;
                    if (state is RestaurantLoginStateLoading) {
                      return const LoadingWidget();
                    }
                    return PrimaryButton(
                      text: widget.buttonText,
